...
std::uint64_t g_prng_state = UINT64_C(0xaaa266ba1afa7f15);
...
std::uint64_t NextRandom64() noexcept {
    ...
    x ^= x >> 11;
    g_prng_state = x;
    return x * UINT64_C(0x2545f4ffff6cdd1d);
}
...
/*
bool CreateHarnessResources() {
    g_resources.desktop_window = nullptr;
    ...
    g_resources.memory_dc = ::CreateCompatibleDC(screen);
    if (!g_resources.memory_dc) {
    ...
    g_resources.bitmap =
        ::CreateCompatibleBitmap(screen, 2048, 2048);
    ...
    g_resources.old_bitmap =
        (HBITMAP)::SelectObject(g_resources.memory_dc,
                                g_resources.bitmap);

    ::ReleaseDC(nullptr, screen);

    // The harness should use THIS HDC from now on.
    g_resources.screen_dc = g_resources.memory_dc;
...
static void RunWarmup(const WarmupConfig& config,
    ...
    */
    ...
    for (std::size_t iteration = 0;
        ...
        generated_ntgdi::InitializeHandlePools();
